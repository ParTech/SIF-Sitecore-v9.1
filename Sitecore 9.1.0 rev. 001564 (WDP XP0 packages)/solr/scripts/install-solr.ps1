...
Write-Host "Running install-solr.ps1 ..."

if ($parameters.Solr.Install -eq $false)
{
    Write-Host "Skipping SOLR installation ..."
    Write-Host "Finished install-solr.ps1 ..."
    Exit 0
}

$solrVersion = $parameters.Solr.Version
$installFolder = $parameters.Solr.InstallFolder
$solrPort = $parameters.Solr.Port
$solrHost = $parameters.Solr.Host
$solrSSL = $parameters.Solr.SSL
$nssmVersion = $parameters.Solr.NssmVersion
$JREVersion = $parameters.Solr.JREVersion
...
$solr_installed = Test-Path -Path "$installFolder\solr-$solrVersion"

if ($solr_installed -eq $false)
{
    $java_installed = Test-Path -Path "C:\Program Files\Java\jre$JREVersion"
    if ($java_installed -eq $false)
    {
        throw "Java Runtime Environment version $JREVersion is not installed!`r`nInstall this first or update the version number in your parameters file and then run this script again.`r`n `r`n"
    }
...
{
    Write-Host "SOLR already installed ..."
}
